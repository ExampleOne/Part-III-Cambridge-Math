...
\begin{table}[H]
  ...
  + i \epsilon} e^{-ip \cdot (x - y)}, \ \epsilon \to 0^+$ & \\ \hline
      4-vector mode expansion & $A_\mu(x) = \int \frac{d^3p}{(2\pi)^3} \frac{1}{\sqrt{2 |p|}} \sum_{\lambda = 0}^3
                                \epsilon_\mu^{(\lambda)}(p)(a_p^\lambda e^{-i p \cdot x} + 
        a_p^{\lambda \dagger} e^{ip \cdot x})$ & this is used for photons in QED. 
        Note $[a_p^\lambda, a_q^{\rho \dagger}] = - \eta^{\lambda \rho} (2 \pi)^3 \delta(p - q)$ \\ \hline
      QED Lagrangian coupling a Gauge theory to a scalar field & $\mathcal{L} = -\frac{1}{4} F_{\mu \nu} F^{\mu \nu} +
                                                                 \bar{\psi} (i \gamma^\mu \partial_\mu - m) \psi - 
        e \bar{\psi} \gamma^\mu \psi A_\mu$ & \\ \hline
    \end{tabular}
  \end{adjustbox}
\end{table}
